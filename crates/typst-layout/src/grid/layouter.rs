...
impl<'a> GridLayouter<'a> {
    ...
    fn measure_columns(
        ...
        let mut align_engine = AlignPointsEngine::new(self.rcols.len());
        ...
        for (x, (&col, rcol)) in self.grid.cols.iter().zip(&mut self.rcols).enumerate() {
            match col {
                ...
                Sizing::Rel(v) => {
                    ...
                    *rcol = resolved;
                    rel += resolved;
                    align_engine.set_zone_size(x, resolved);
                }
                Sizing::Fr(v) => fr += v,
            }
        }
        ...
        for (x, &rcol) in self.rcols.iter().enumerate() {
            for y in 0..self.grid.rows.len() {
                ...
                if parent.x != x {
                    // Don't check the width of colspans more than once.
                    continue;
                }
                if parent.y != y {
                ...
                for (point_x, id) in frame.horizontal_align_points() {
                    align_engine.add_point(
                        id.clone(),
                        parent.x..parent.x + self.grid.effective_colspan_of_cell(cell),
                        point_x,
                        frame.width() - point_x,
                    );
    ...
    fn measure_auto_columns(
        ...
        let mut align_engine = AlignPointsEngine::new(self.grid.cols.len());
        ...
        for (x, &col) in self.grid.cols.iter().enumerate() {
            ...
            //    continue;
            //}
            //if matches!(col, Sizing::Fr(_)) {
            //    continue;
            //}
            if matches!(col, Sizing::Rel(_)) {
                align_engine.set_zone_size(x, self.rcols[x]);
            }
            ...
            for y in 0..self.grid.rows.len() {
                ...
                if parent.x != x || parent.y != y {
                    // Don't handle the same cell several times.
                    continue;
                }
                let cell = self.grid.cell(parent.x, parent.y).unwrap();
                //let colspan = self.grid.effective_colspan_of_cell(cell);
                // Needed with colspan+align points?
                //if colspan > 1
    ...
    fn layout_single_row(
        ...
        let mut vertical_align_engine = AlignPointsEngine::new(1);
            ...
            if let Some(cell) = self.grid.cell(x, y) {
                ...
                if cell.rowspan.get() == 1 {
                    ...
                    if let Some((point_x, id)) = frame.horizontal_align_points().next() {
                        if let Some(engine) = &mut grid_infos.horiz_align_engine {
                            let position = engine.get_position(id);
                            dx = position - point_x - pos.x;
                            dw = engine.get_extra_space(id);
                        }
                    }
